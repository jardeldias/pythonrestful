...
class CursoControllerResource(Resource):
    ...
    def post(self):
        ...
        if validate:
            ...
            novo_curso = curso_dto.CursoDTO(nome=nome, descricao=descricao)
    ...
    def put(self, id):
        ...
        if validate:
            ...
            nome = request.json['nome']
            descricao = request.json['descricao']
            novo_curso_alterado = curso_dto.CursoDTO(nome=nome, descricao=descricao)
            curso_service.atualizar_curso(curso, novo_curso_alterado)
            curso_atualizado = curso_service.listar_curso_by_id(id)
            return make_response(curso_schema_var.jsonify(curso_atualizado), 200)
    ...
    def delete(self, id):
        ...
        return make_response('Curso expluido com sucesso',204)
...
api.add_resource(CursoControllerResource,'/curso', endpoint='cursoGetPost')
api.add_resource(CursoControllerResource,'/curso/<int:id>', endpoint='cursoPutDelte', methods=['PUT', 'DELETE'])
api.add_resource(CursoDetailControllerResource,'/curso/<int:id>', endpoint='cursoGetId')
